...
public class DemoTests {
    ...
    @ru.t1.testrunner.annotations.Test("successfull test")
    @Order(1)
    void success() {
        if (counter <= 0) {
    ...
    @ru.t1.testrunner.annotations.Test("failed assertion")
    @Order(2)
    void failed() {
        throw new TestAssertionError("Expected value to be equal");
    }

    @ru.t1.testrunner.annotations.Test("unexpected exception")
    @Order(3)
    void error() {
        throw new RuntimeException("Unexpected error");
    }

    @ru.t1.testrunner.annotations.Test("disabled test")
    @Disabled
    void skipped() {
    }
}
